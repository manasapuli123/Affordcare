...
function initialState() {
  return {
    ...
    docs: {
      ...
      consent: { uploaded: false, filename: "" },
    },
    appStatusIndex: -1,
    notifications: [
      {
...
export function useAffordCare() {
  ...
  const wizardNext = useCallback(() => {
    const next = Math.min(3, state.wizardStep + 1);
      ...
      notifications: [
        {
          ...
          body: `We received your enrollment for ${program ? program.name : "your program"}.`,
          time: "Just now",
          read: false,
        },
        ...s.notifications,
  ...
  const goToDocuments = useCallback(() => {
    const complete = Object.values(state.docs).every((d) => d.uploaded);
    setState((s) => {
      if (complete || s.notifiedMissingDoc) return { ...s, stage: "documents" };
      return {
        ...s,
        stage: "documents",
        notifiedMissingDoc: true,
        notifications: [
          {
            id: Date.now(),
            type: "missing_doc",
            title: "Missing document",
            body: "Upload your remaining documents to keep your application moving.",
            time: "Just now",
            read: false,
          },
          ...s.notifications,
  ...
  const nextAction = (() => {
    ...
    if (!state.enrollment.submitted) {
      if (state.personal.name)
        return {
          stage: "financial-assistance",
          label: `Resume your enrollment (step ${state.wizardStep} of 3)`,
      ...
      return {
        ...
        desc: "A short 3-step form connects you to your selected program.",
        icon: "ClipboardList",
      };
    }
    if (!allDocsUploaded)
      return {
        ...
        desc: "Your program enrollment needs a few documents before it can be reviewed.",
        icon: "UploadCloud",
      };
